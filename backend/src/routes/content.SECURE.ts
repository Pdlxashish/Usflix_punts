...
// Helper function to get admin's user ID
async function getAdminUserId(adminUsername: string): Promise<number | null> {
...
router.post("/collections", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const userId = await getAdminUserId(req.auth!.username);
    if (!userId) {
...
/** GET /api/media — List all media items for authenticated user */
router.get("/media", requireUserAuth, async (req: Request, res: Response) => {
  try {
    const { rows } = await pool.query(
      "SELECT * FROM media_items WHERE user_id = $1 ORDER BY sort_rank ASC",
      [req.userAuth!.userId]
    );
...
router.get("/media/:id", requireUserAuth, async (req: Request, res: Response) => {
  try {
    const { rows } = await pool.query(
      ...
      [req.params.id, req.userAuth!.userId]
    );
    if (rows.length === 0) {
