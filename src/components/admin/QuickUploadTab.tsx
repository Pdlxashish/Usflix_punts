...
import { Upload as UploadIcon, ImagePlus, Film, Mic, X, CheckCircle, AlertCircle, Loader2, Zap, Plus } from "lucide-react";
...
export function QuickUploadTab() {
    ...
    for (const file of Array.from(fileList)) {
      ...
      if (ACCEPTED_VIDEO_MIMES.includes(file.type) || ACCEPTED_VIDEO_EXTS.includes(ext)) {
        if (file.size > MAX_VIDEO_SIZE) {
          newFiles.push({ file, progress: 0, status: "error", error: "File exceeds 4 GB limit.", type: "video" });
          continue;
        }
        ...
        if (file.size > MAX_AUDIO_SIZE) {
          newFiles.push({ file, progress: 0, status: "error", error: "File exceeds 100 MB limit.", type: "voice" });
          continue;
        }
        ...
        if (!fmt) {
          newFiles.push({ file, progress: 0, status: "error", error: `Unsupported format.`, type: "image" });
          continue;
        }
        if (file.size > MAX_IMAGE_SIZE) {
          newFiles.push({ file, progress: 0, status: "error", error: "File exceeds 50 MB limit.", type: "image" });
          continue;
        }
  ...
  const uploadFiles = async () => {
    ...
    for (const pf of pending) {
      ...
      try {
        // Upload file
        const uploadData = await new Promise<{ url: string; thumbnailUrl?: string; duration?: number }>((resolve, reject) => {
        ...
        if (pf.type === "video") {
          ...
          mediaBody.photos = [uploadData.url];
        }
    ...
    return (
      <div className="bg-card/50 border border-border/60 rounded-xl p-6 text-center py-16">
        ...
        <p className="text-muted-foreground mb-2">
          <span className="text-foreground font-medium">{uploadedCount} file{uploadedCount !== 1 ? "s" : ""}</span> uploaded successfully
        </p>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {files.map((f, i) => (
                ...
                >
                  {f.preview ? (
                    <img src={f.preview} alt="" className="w-12 h-12 rounded object-cover shrink-0" />
                  ) : (
          ...
          <div className="flex items-center gap-4 pt-2">
            ...
            >
              {uploading
                ...
                : <><Zap className="h-4 w-4" /> Quick Upload {files.filter(f => f.status === "pending").length} File{files.filter(f => f.status === "pending").length !== 1 ? "s" : ""}</>
              }
            </button>
          </div>
        )}
