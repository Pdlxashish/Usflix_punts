...
import { Upload as UploadIcon, ImagePlus, Film, Mic, X, CheckCircle, AlertCircle, Loader2, Plus } from "lucide-react";
...
const ACCEPTED_VIDEO_MIMES = ["video/mp4", "video/quicktime", "video/x-matroska", "video/webm", "video/3gpp", "video/mpeg"];
const ACCEPTED_VIDEO_EXTS  = [".mp4", ".mov", ".mkv", ".webm", ".3gp", ".mpeg", ".mpg", ".avi"];
const ACCEPTED_IMAGE_MIMES = ["image/jpeg", "image/png", "image/webp", "image/gif", "image/heic", "image/heif", "image/avif"];
const ACCEPTED_IMAGE_EXTS  = [".jpg", ".jpeg", ".png", ".webp", ".gif", ".heic", ".heif", ".avif"];
const ACCEPTED_AUDIO_MIMES = ["audio/mpeg", "audio/mp3", "audio/wav", "audio/ogg", "audio/aac", "audio/x-m4a", "audio/mp4", "audio/webm"];
...
async function detectImageFormat(file: File): Promise<string | null> {
  ...
  const extMap: Record<string, string> = { jpg:"JPEG", jpeg:"JPEG", png:"PNG", gif:"GIF", webp:"WebP", heic:"HEIC", heif:"HEIF", avif:"AVIF" };
...
export function UploadTab() {
    ...
    for (const file of Array.from(fileList)) {
      ...
      if (uploadType === "video") {
        if (!ACCEPTED_VIDEO_MIMES.includes(file.type) && !ACCEPTED_VIDEO_EXTS.includes(ext)) {
          newFiles.push({ file, progress: 0, status: "error", error: `Unsupported format. Accepted: MP4, MOV, MKV, WebM, 3GP` });
          continue;
        }
        if (file.size > MAX_VIDEO_SIZE) {
          newFiles.push({ file, progress: 0, status: "error", error: "File exceeds 4 GB limit." });
          continue;
        }
        ...
        if (!ACCEPTED_AUDIO_MIMES.includes(file.type) && !ACCEPTED_AUDIO_EXTS.includes(ext)) {
          newFiles.push({ file, progress: 0, status: "error", error: `Unsupported format. Accepted: MP3, WAV, OGG, AAC, M4A` });
          continue;
        }
        if (file.size > MAX_AUDIO_SIZE) {
          newFiles.push({ file, progress: 0, status: "error", error: "File exceeds 100 MB limit." });
          continue;
        }
        newFiles.push({ file, progress: 0, status: "pending" });
        ...
        if (!fmt) {
          newFiles.push({ file, progress: 0, status: "error", error: `Unsupported format. Accepted: JPEG, PNG, WebP, GIF, HEIC, HEIF, AVIF.` });
          continue;
        }
        if (file.size > MAX_IMAGE_SIZE) {
          newFiles.push({ file, progress: 0, status: "error", error: "File exceeds 50 MB limit." });
          continue;
        }
  ...
  const uploadFiles = async () => {
    ...
    for (const pf of pending) {
      setFiles((prev) => prev.map((f, i) => i === pf.idx ? { ...f, status: "uploading" as const, progress: 0 } : f));
      ...
      try {
        ...
        setFiles((prev) => prev.map((f, i) => i === pf.idx ? { ...f, status: "done" as const, progress: 100 } : f));
      } catch (err: any) {
        setFiles((prev) => prev.map((f, i) => i === pf.idx ? { ...f, status: "error" as const, error: err.message || "Upload failed" } : f));
      }
    }
    ...
    if (uploadedUrls.length > 0) {
      try {
        ...
        const mediaBody: Record<string, unknown> = {
          ...
          thumbnail: isVoice ? undefined : uploadedUrls[0],
          category: albumToUse, // Use the selected or newly created album
          status: "ready",
        };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (files.filter((f) => f.status !== "error").length === 0) errs.files = "Add at least one file.";
    setFormErrors(errs);
  ...
  const reset = () => {
    ...
    setFiles([]); setTitle(""); setCollection(""); setTagline("");
    setFormErrors({}); setSubmitted(false); setUploading(false);
  ...
  return (
    <div className="bg-card/50 border border-border/60 rounded-xl p-6">
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-6">
        {/* Upload type toggle */}
        <div className="flex gap-1 bg-input/50 border border-border/60 rounded-lg p-1 w-fit" role="group" aria-label="Upload type">
          <button
            type="button"
          ...
          <button
            type="button"
            onClick={() => { setUploadType("video"); setFiles([]); }}
          ...
          <button
            type="button"
            onClick={() => { setUploadType("voice"); setFiles([]); }}
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              value={title}
              onChange={(e) => { setTitle(e.target.value); if (formErrors.title) setFormErrors((er) => ({ ...er, title: "" })); }}
            ...
            />
            {formErrors.title && <p className="text-xs text-destructive flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {formErrors.title}</p>}
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            value={tagline}
            onChange={(e) => { setTagline(e.target.value); if (formErrors.tagline) setFormErrors((er) => ({ ...er, tagline: "" })); }}
          ...
          <div className="flex justify-between">
            {formErrors.tagline
              ? <p className="text-xs text-destructive flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {formErrors.tagline}</p>
              : <span />
            }
        ...
        <div className="space-y-1.5">
          <label className="text-sm font-medium text-foreground/80">
            {uploadType === "video" ? "Video Files" : uploadType === "voice" ? "Audio Files" : "Photos"}{" "}
            <span className="text-primary">*</span>
            <span className="text-muted-foreground font-normal ml-2">({files.filter((f) => f.status !== "error").length} selected)</span>
          </label>
          <label
          ...
          >
            ...
            <p className="font-display text-lg text-foreground">
              Drop {uploadType === "video" ? "videos" : uploadType === "voice" ? "audio" : "photos"} here
            </p>
          ...
          {formErrors.files && <p className="text-xs text-destructive flex items-center gap-1"><AlertCircle className="h-3 w-3" /> {formErrors.files}</p>}
        </div>
            ...
            {files.map((f, i) => (
              ...
              >
                ...
                <div className="shrink-0 flex items-center gap-2">
                  {f.status === "uploading" && <span className="text-xs text-primary tabular-nums">{f.progress}%</span>}
        ...
        <div className="flex items-center gap-4 pt-2">
          ...
          {(files.length > 0 || title || tagline) && !uploading && (
            <button type="button" onClick={reset} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
              Clear all
            </button>
          )}
        </div>
        ...
        <p className="text-xs text-muted-foreground bg-card/40 border border-border/40 rounded-md px-4 py-3 mt-4">
          Files are uploaded to the server and saved to the database. iPhone photos (HEIC) and videos (MOV) are supported.
        </p>
      </form>
    </div>
  );
