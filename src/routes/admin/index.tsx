...
const ALBUM_SECTIONS: AdminNavSection<AlbumSection>[] = [
  {
    ...
    shortLabel: "Create",
    icon: Plus,
    description: "Add a new album or sub-album",
  },
  {
    id: "manage",
    label: "Manage Albums",
    shortLabel: "Albums",
    icon: FolderTree,
    description: "Rename, edit, or delete albums",
  },
  {
    id: "media",
    label: "All Media",
    shortLabel: "Media",
    icon: Film,
    description: "Edit, feature, or delete photos and videos",
  },
];
...
function AdminPanel() {
  ...
  const [tab, setTab] = useState<
    "customization" | "profiles" | "albums" | "upload" | "romance" | "account"
  >("customization");
  ...
  const handleMediaFileUpload = async (mediaId: string, file: File) => {
    ...
    try {
      ...
      // Build the update payload with correct types
      ...
      if (isVideo) {
        ...
        updatePayload.photos = [{ src: uploadData.url, caption: mediaItem.title }];
      }

      // Update via backend using BACKEND_URL so it works in Capacitor builds too
      const updateRes = await fetch(`${BACKEND_URL}/api/media/${mediaId}`, {
        method: "PUT",
        credentials: "include",
  ...
  const handleThumbnailUpload = async (mediaId: string, file: File) => {
    ...
    try {
      ...
      // Update only the thumbnail using BACKEND_URL
      const updateRes = await fetch(`${BACKEND_URL}/api/media/${mediaId}`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="pt-20 sm:pt-24 pb-20 sm:pb-24 px-3 sm:px-6 lg:px-12 max-w-6xl mx-auto min-w-0 overflow-x-hidden">
        ...
        <div className="space-y-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage the love letters, jar, mood board, and milestones shown on the homepage.
            </p>
          </div>
            ...
            <div className="bg-card/50 border border-border/60 rounded-xl p-4 sm:p-6 min-w-0">
              {colError && (
                <p className="text-sm text-destructive mb-3 flex items-center gap-1">
                  <AlertCircle className="h-4 w-4" /> {colError}
                </p>
              )}
              ...
              <button
                onClick={async () => {
                  ...
                  const r = await createCollection({
                    ...
                    parentId: newColParent || undefined,
                  });
                  if (r.ok) {
                    setNewColName("");
                    setNewColDesc("");
                    ...
                    setColError(r.error ?? "Failed to create.");
                    toast.error(r.error ?? "Failed to create album");
                  }
                }}
          ...
          {albumSection === "manage" && (
            <div className="bg-card/50 border border-border/60 rounded-xl p-4 sm:p-6 min-w-0">
              ...
              <div className="space-y-2">
                {collections.map((col) => {
                  const childCount = mediaItems.filter(
                    (m) => m.category.toLowerCase() === col.name.toLowerCase(),
                  ).length;
                  const isEditing = editingCol === col.id;
                  return (
                    <div
                      key={col.id}
                      className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 bg-input/50 border border-border/40 rounded-lg px-3 sm:px-4 py-3 min-w-0"
                    >
                      {col.parentId && (
                        <span className="text-muted-foreground text-xs sm:mr-0">↳</span>
                      )}
                      {isEditing ? (
                        <div className="flex-1 flex flex-col sm:flex-row gap-2 min-w-0 w-full">
                          <input
                            ...
                            className="flex-1 min-w-0 bg-input border border-border rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-ring"
                          />
                          <input
                            value={editColDesc}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {mediaItems.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className={`bg-input/30 border rounded-lg overflow-hidden hover:border-primary/40 transition-colors ${isSelected ? "border-primary border-2" : "border-border/40"}`}
                    >
                      {/* Preview Thumbnail */}
                      <div className="relative aspect-video bg-muted">
                        ...
                        <div
                          className={`absolute inset-0 flex items-center justify-center ${item.thumbnail ? "hidden" : ""}`}
                        >
                          {item.type === "video" ? (
                          ...
                          <div className="space-y-3">
                            ...
                            <div>
                              <label className="text-xs text-muted-foreground mb-1 block">
                                Thumbnail Image{" "}
                                {item.type === "video" && <span className="text-primary">*</span>}
                              </label>
                              ...
                              {editMediaThumbnail && (
                                <div className="mt-2">
                                  <img
                                    src={getMediaUrl(editMediaThumbnail)}
                            ...
                            <div>
                              <label className="text-xs text-muted-foreground mb-1 block">
                                Change {item.type === "video" ? "Video" : "Photo"}
                              </label>
                              <label className="inline-flex items-center gap-2 border border-border px-3 py-2 rounded text-sm hover:bg-card hover:border-primary/40 transition-colors cursor-pointer w-full justify-center">
                                <Upload className="h-3.5 w-3.5" />
                                {uploadingMediaFile
                                  ? "Uploading..."
                                  : `Upload New ${item.type === "video" ? "Video" : "Photo"}`}
                                <input
                                  type="file"
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                onClick={async () => {
                                  const r = await updateMediaItem(item.id, {
                                    title: editMediaTitle,
                                    description: editMediaDesc,
                                    tagline: editMediaTagline,
                                    sortRank: editMediaRank,
                                    thumbnail: editMediaThumbnail || undefined,
                                  });
                                  if (r.ok) {
                              ...
                              <button
                                ...
                                className="flex-1 border border-border px-3 py-2 rounded text-sm hover:bg-card transition-colors"
                              >
                                Cancel
                              </button>
                            </div>
      ...
      {deleteColTarget && deleteColItem && (
        <ConfirmDialog
          ...
          options={[
            {
              label: deleteColItem.parentId
                ? "Move items to parent album"
      ...
      {lastDeleted && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[150] flex items-center gap-3 bg-card border border-border rounded-lg px-5 py-3 shadow-[var(--shadow-card)] animate-in slide-in-from-bottom duration-300">
          ...
          <button
            onClick={() => {
              setLastDeleted(null);
              toast.info("Undo is not available for permanent deletions");
            }}
  ...
  const sections: { id: RomanceSection; label: string; emoji: string; desc: string }[] = [
    {
      ...
      label: "Love Letters",
      emoji: "💌",
      desc: "Flip cards with handwritten notes",
    },
    ...
    { id: "moodboard", label: "Mood Board", emoji: "🖼️", desc: "Aesthetic photo grid" },
    {
      ...
      label: "First Times",
      emoji: "✨",
      desc: "Relationship milestone timeline",
    },
    { id: "quiz", label: "Quiz", emoji: "🧠", desc: "How well do you know us? quiz questions" },
    {
      ...
      label: "Bucket List",
      emoji: "🎯",
      desc: "Dreams and adventures to do together",
    },
    { id: "mood", label: "Daily Mood", emoji: "😊", desc: "Set today's mood and message" },
    {
      id: "greetings",
      label: "Time Greetings",
      emoji: "🌙",
      desc: "Good morning/evening messages based on time",
    },
    {
      id: "playlist",
      label: "Our Playlist",
      emoji: "🎵",
      desc: "Songs that remind you of each other",
    },
